...
export default function token() {
  ...
  const handleSubmit = async (event) => {
    ...
    let new_password = event.target.confirm_password.value;
    ...
    fetch('../../api/reset-password', {
      ...
      body: JSON.stringify({
        ...
        new_password: event.target.confirm_password.value,
      }),
    }).then(async (response) => {
      ...
      <div className="m-auto w-full max-w-md bg-white py-8 px-8 shadow  sm:rounded-lg sm:px-10">
        <form onSubmit={handleSubmit}>
          <div className="text-center">
            ...
            <p className="mt-3">Enter a new password to reset your account.</p>
          </div>
